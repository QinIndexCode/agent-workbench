...
const backendPort = Number.parseInt(process.env.IMPROVEMENT_AUDIT_BACKEND_PORT ?? '3911', 10);
const frontendPort = Number.parseInt(process.env.IMPROVEMENT_AUDIT_FRONTEND_PORT ?? '5873', 10);
const mockProviderPort = Number.parseInt(process.env.IMPROVEMENT_AUDIT_MOCK_PROVIDER_PORT ?? '4311', 10);
...
async function main() {
  ...
  try {
    ...
    const review = spawnNpm(['run', 'improvement-proposal-audit', '-w', 'frontend', '--'], {
      ...
      FRONTEND_IMPROVEMENT_AUDIT_MODE: 'full'
    });
    const readReviewLogs = collectOutput(review, 'frontend-improvement-proposal-audit');
    ...
    if (exitCode !== 0) {
      ...
      throw new Error(`frontend improvement proposal audit failed (${exitCode})\n${logs.stdout}\n${logs.stderr}`);
    }
  } catch (error) {
