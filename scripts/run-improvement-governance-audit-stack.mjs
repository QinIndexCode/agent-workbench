...
const backendPort = Number.parseInt(process.env.IMPROVEMENT_GOVERNANCE_AUDIT_BACKEND_PORT ?? '3931', 10);
const frontendPort = Number.parseInt(process.env.IMPROVEMENT_GOVERNANCE_AUDIT_FRONTEND_PORT ?? '5893', 10);
const mockProviderPort = Number.parseInt(process.env.IMPROVEMENT_GOVERNANCE_AUDIT_MOCK_PROVIDER_PORT ?? '4331', 10);
const frontendBaseUrl = `http://127.0.0.1:${frontendPort}`;
const backendBaseUrl = `http://127.0.0.1:${backendPort}`;
const mockProviderUrl = `http://127.0.0.1:${mockProviderPort}`;
const backendRootDir = path.join(rootDir, '.codex-run', 'tmp', `improvement-governance-audit-${Date.now()}`);
...
async function main() {
  ...
  try {
    ...
    const audit = spawnNode(path.join(rootDir, 'scripts', 'improvement-governance-audit.mjs'), {
      FRONTEND_BASE_URL: frontendBaseUrl,
      IMPROVEMENT_GOVERNANCE_AUDIT_BACKEND_URL: backendBaseUrl,
      IMPROVEMENT_GOVERNANCE_AUDIT_MOCK_PROVIDER_URL: mockProviderUrl,
      IMPROVEMENT_GOVERNANCE_AUDIT_BACKEND_ROOT_DIR: backendRootDir,
    });
    const readAuditLogs = collectOutput(audit, 'improvement-governance-audit');
    ...
    if (exitCode !== 0) {
      ...
      throw new Error(`improvement governance audit failed (${exitCode})\n${logs.stdout}\n${logs.stderr}`);
    }
  } catch (error) {
