...
const backendPort = Number.parseInt(process.env.LESSON_MEMORY_DEDUPE_AUDIT_BACKEND_PORT ?? '3941', 10);
const mockProviderPort = Number.parseInt(process.env.LESSON_MEMORY_DEDUPE_AUDIT_MOCK_PROVIDER_PORT ?? '4341', 10);
const backendBaseUrl = `http://127.0.0.1:${backendPort}`;
const mockProviderUrl = `http://127.0.0.1:${mockProviderPort}`;
const backendRootDir = path.join(rootDir, '.codex-run', 'tmp', `lesson-memory-dedupe-audit-${Date.now()}`);
...
async function main() {
  ...
  try {
    ...
    const audit = spawnNode(path.join(rootDir, 'scripts', 'lesson-memory-dedupe-audit.mjs'), {
      LESSON_MEMORY_DEDUPE_AUDIT_BACKEND_URL: backendBaseUrl,
      LESSON_MEMORY_DEDUPE_AUDIT_MOCK_PROVIDER_URL: mockProviderUrl,
    });
    const readAuditLogs = collectOutput(audit, 'lesson-memory-dedupe-audit');
    ...
    if (exitCode !== 0) {
      ...
      throw new Error(`lesson memory dedupe audit failed (${exitCode})\n${logs.stdout}\n${logs.stderr}`);
    }
  } catch (error) {
