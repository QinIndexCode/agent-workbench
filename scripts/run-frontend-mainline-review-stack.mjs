...
const backendPort = Number.parseInt(process.env.MAINLINE_REVIEW_BACKEND_PORT ?? '3711', 10);
const frontendPort = Number.parseInt(process.env.MAINLINE_REVIEW_FRONTEND_PORT ?? '5673', 10);
const mockProviderPort = Number.parseInt(process.env.MAINLINE_REVIEW_MOCK_PROVIDER_PORT ?? '4111', 10);
...
async function main() {
  ...
  try {
    ...
    const review = spawnNpm(['run', 'mainline-review', '-w', 'frontend', '--'], {
      FRONTEND_BASE_URL: frontendBaseUrl,
      FRONTEND_MAINLINE_REVIEW_BACKEND_URL: backendBaseUrl,
      FRONTEND_MAINLINE_REVIEW_MOCK_PROVIDER_URL: mockProviderUrl
    });
    const readReviewLogs = collectOutput(review, 'frontend-mainline-review');
    ...
    if (exitCode !== 0) {
      ...
      throw new Error(`frontend mainline review failed (${exitCode})\n${logs.stdout}\n${logs.stderr}`);
    }
  } catch (error) {
