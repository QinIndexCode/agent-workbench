...
const preferredBackendPort = Number.parseInt(process.env.DELEGATION_LIVE_REVIEW_BACKEND_PORT ?? '3811', 10);
const preferredFrontendPort = Number.parseInt(process.env.DELEGATION_LIVE_REVIEW_FRONTEND_PORT ?? '5773', 10);
const reportPath =
  process.env.FRONTEND_DELEGATION_LIVE_REVIEW_REPORT ??
  path.resolve(rootDir, '.codex-run', 'logs', 'frontend-delegation-live-review.json');
const configuredBackendRootDir = process.env.DELEGATION_LIVE_REVIEW_BACKEND_ROOT_DIR?.trim() ?? '';
const backendRootDir = configuredBackendRootDir
  ...
  : createIsolatedBackendRuntimeRoot(rootDir, 'frontend-delegation-live-review');
...
async function main() {
  ...
  const backend = spawnNpm(['run', 'start', '-w', 'backend'], {
    ...
    BACKEND_NEW_RUNTIME_DELEGATION_ENABLED: '1',
    BACKEND_NEW_RUNTIME_DELEGATION_MAX_DEPTH: '1',
    BACKEND_NEW_RUNTIME_DELEGATION_MAX_ACTIVE_CHILDREN_PER_TASK: '1',
  ...
  try {
    ...
    const review = spawnNpm(['run', 'delegation-live-review', '-w', 'frontend', '--'], {
      ...liveEnv,
      FRONTEND_BASE_URL: frontendBaseUrl,
      FRONTEND_DELEGATION_LIVE_REVIEW_BACKEND_URL: backendBaseUrl,
      FRONTEND_DELEGATION_LIVE_REVIEW_REPORT: reportPath
    });
    const readReviewLogs = collectOutput(review, 'frontend-delegation-live-review');
    ...
    if (exitCode !== 0) {
      ...
      throw new Error(`frontend delegation live review failed (${exitCode})\n${logs.stdout}\n${logs.stderr}`);
    }
  } catch (error) {
