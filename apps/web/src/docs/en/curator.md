# Curator

Curator explains why Agent Workbench recommends, blocks, or groups a skill candidate.

## What to look at first

- **Reason**: why the item is here
- **Evidence**: source tasks, tools, or patterns behind the suggestion
- **Blocked reasons**: why Agent Workbench refused to promote it
- **Duplicate basis**: why multiple skills were grouped together

## Recommended actions

- **Activate** only when the candidate is clearly reusable
- **Suspend** if an active skill starts drifting or causing false matches
- **Merge duplicates** when the grouped skills truly describe the same workflow

## A strong candidate usually has

- repeated successful tasks behind it
- a stable multi-step tool sequence
- reusable wording instead of task-specific output

If these are weak, keep it as candidate or not promoted.
