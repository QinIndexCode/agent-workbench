...
describe("Workbench components", () => {
  ...
  it("renders task list status as text", () => {
    ...
    render(<TaskList tasks={[task]} selectedId="task_1" onSelect={onSelect} />);
  ...
  it("renders compact governance rows", () => {
    render(<CompactList title="Skills" rows={[{ id: "skill_1", label: "Host observation", meta: "enabled" }]} />);
  ...
  it("creates a task and resolves an approval from the app shell", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === "/api/experiences" || url === "/api/skills") return jsonResponse([]);
