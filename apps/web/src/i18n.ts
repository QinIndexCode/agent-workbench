...
const copy = {
  "zh-CN": {
    shell: {
      ...
      collapseFolder: (name: string) => `收起 ${name}`,
      defaultFolder: "默认文件夹",
      addFolder: "新建文件夹",
      editFolder: "编辑文件夹",
      deleteFolder: "删除文件夹",
    ...
    permissions: {
      ...
      coverageNotice: "破坏性操作默认仍受严格约束。只有全局明确允许时，删除、覆盖和终止进程这类行为才会跳过逐次确认。",
      granted: "已全局允许",
      notGranted: "需要审批",
      autoAllowed: "自动通过",
      ...
      preferencesNotice: "偏好页只改变展示、语气和本地存储行为，不会替你放宽工具权限。",
      ...
      preferencesPrimer: {
        summary: "偏好页负责个性化和本地行为，不负责替代权限审批。",
        focus: "调整语言、主题、回答风格、启动页面和存储清理等个人工作习惯。",
        impact: "会影响你看到的界面、回复风格以及本地数据如何保存，但不会改变工具风险边界。",
        nextStep: "先定语言和主题，再根据需要调节展示思考、自动注入 Skill 和本地存储习惯。"
      },
  ...
  "en-US": {
    ...
    permissions: {
      ...
      coverageNotice: "Destructive actions stay tightly controlled by default. Delete, overwrite, and process termination only skip confirmation when you have explicitly granted them globally.",
      ...
      preferencesNotice: "Preferences change presentation, tone, and local storage behavior. They do not relax tool permissions on your behalf.",
      ...
      preferencesPrimer: {
        summary: "Preferences are for personalization and local behavior, not as a shortcut around approvals.",
        focus: "Adjust language, theme, response style, startup behavior, and local storage hygiene for your normal workflow.",
        impact: "Changes affect what you see, how the agent responds, and how local data is stored, without changing tool risk boundaries.",
        nextStep: "Set language and theme first, then tune thinking visibility, skill injection, and storage behavior as needed."
      },
