...
function formatScenarioLine(result: Awaited<ReturnType<typeof runTaskLiveProviderScenarioSuite>>['scenarios'][number]): string {
  return [
    ...
    `verdict=${result.artifactQuality.verdict}`,
    ...
    `quality=${result.artifactQuality.summary}`
  ].join(', ');
}
...
async function main(): Promise<void> {
  ...
  const lines = [
    ...
    `rates: artifactQualityPassRate=${report.totals.artifactQualityPassRate}, liveProviderPassRate=${report.totals.liveProviderPassRate}`,
