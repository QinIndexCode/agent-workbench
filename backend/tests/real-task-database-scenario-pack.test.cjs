...
test('database scenario pack registration stays outside core quality profiles', async () => {
  const {
    ...
    scenarioRequiresStrongLiveModel,
  } = await loadScenarioPacksModule();

  assert.equal(getRealTaskScenarioPackId('database-near-mysql-design'), 'database-design');
  assert.equal(getRealTaskScenarioPackId('database-near-mysql-verify'), 'database-verify');
  assert.equal(scenarioBelongsToPack('database-near-mysql-design', 'database-design'), true);
  assert.equal(scenarioBelongsToAnyPack('database-near-mysql-verify', ['database-design', 'database-verify']), true);
  assert.equal(scenarioRequiresStrongLiveModel('database-near-mysql-design'), true);
  assert.equal(getScenarioClassificationPolicy('database-near-mysql-design').kind, 'prototype_design');
  assert.equal(getScenarioQualityProfileId('database-near-mysql-design'), null);
  assert.equal(getScenarioQualityGateId('database-near-mysql-design'), 'database_near_mysql_design');
  assert.deepEqual(getScenarioProjectKinds('database-near-mysql-design'), ['node']);

  const specs = buildRealTaskScenarioSpecs({ targetExternalPath: 'D:\\AAA' });
  const databaseSpecs = specs.filter((spec) => spec.id.startsWith('database-near-mysql-'));
  assert.deepEqual(databaseSpecs.map((spec) => spec.id), [
    'database-near-mysql-design',
    'database-near-mysql-verify',
  ]);
  assert.equal(databaseSpecs.every((spec) =>
    spec.unit?.qualityProfileId == null && getScenarioQualityGateId(spec.id)?.startsWith('database_near_mysql_')
  ), true);

  assert.equal(classifyScenarioWithPolicy('database-near-mysql-verify', {
    surfacesPass: true,
    lifecycleStatus: 'COMPLETED',
    acceptanceVerdict: 'passed',
    qualityVerdict: 'passed',
    artifactPass: false,
    requiresRuntimeVerificationEvidence: true,
    runtimeVerificationEvidence: false,
...
test('database verify audit treats declared missing package entries as blocking', async () => {
  const {
  ...
  assert.deepEqual(blockingMissingEntryRefs, ['main:src/index.js']);
  assert.deepEqual(progress.packageEntryRefs.missingBlocking, ['main:src/index.js']);
  assert.deepEqual(progress.packageEntryRefs.missingOptional, []);
  assert.equal(progress.prototypeTopLevel.completed, false);
  assert.match(summarizeDatabaseArtifactProgress(progress), /prototype package entry refs broken: main:src\/index\.js/i);
});

test('database package entry diagnostics does not block planned required top-level files', async () => {
  const { getBlockingDatabasePackageEntryRefs, buildDatabaseArtifactProgress } = await loadWaveModule();
  const packageEntryDiagnostics = {
    packageJsonFound: true,
    invalidPackageJson: false,
    parseError: null,
    checkedEntries: [
      { entry: 'scripts.bench', target: 'scripts/bench.js', present: false },
      { entry: 'scripts.dry-run', target: 'scripts/bench.js', present: false },
    ],
    missingEntryRefs: ['scripts.bench:scripts/bench.js', 'scripts.dry-run:scripts/bench.js'],
    missingRequiredEntries: [],
  };
  const blockingMissingEntryRefs = getBlockingDatabasePackageEntryRefs(packageEntryDiagnostics, {
    scenarioId: 'database-near-mysql-design',
  });
  const progress = buildDatabaseArtifactProgress([
    'database-lab/design/README.md',
    'database-lab/design/architecture.md',
    ...
    packageEntryDiagnostics,
    blockingMissingEntryRefs,
    optionalMissingEntryRefs: packageEntryDiagnostics.missingEntryRefs,
    scenarioId: 'database-near-mysql-design',
  });

  assert.deepEqual(blockingMissingEntryRefs, []);
  assert.equal(progress.prototypeTopLevel.completed, false);
  assert.ok(progress.prototypeTopLevel.missing.includes('database-lab/prototype/scripts/bench.js'));
  assert.deepEqual(progress.packageEntryRefs.missingBlocking, []);
});

test('database package entry diagnostics does not block planned prototype src entry refs', async () => {
  const { getBlockingDatabasePackageEntryRefs, buildDatabaseArtifactProgress } = await loadWaveModule();
  const packageEntryDiagnostics = {
    packageJsonFound: true,
    invalidPackageJson: false,
    parseError: null,
    checkedEntries: [
      { entry: 'main', target: 'src/storage-engine.js', present: false },
      { entry: 'scripts.bench', target: 'scripts/bench.js', present: true },
    ],
    missingEntryRefs: ['main:src/storage-engine.js'],
    missingRequiredEntries: [],
  };
  const blockingMissingEntryRefs = getBlockingDatabasePackageEntryRefs(packageEntryDiagnostics, {
    scenarioId: 'database-near-mysql-design',
  ...
  assert.deepEqual(blockingMissingEntryRefs, []);
  assert.deepEqual(progress.packageEntryRefs.missingBlocking, []);
  assert.deepEqual(progress.packageEntryRefs.missingOptional, ['main:src/storage-engine.js']);
  assert.equal(progress.prototypeTopLevel.completed, true);
  assert.equal(progress.prototypeModules.completed, false);
  assert.equal(progress.nextStage, 'prototype_modules');
});
...
test('database continue no-progress guard trips after repeated duplicate no-tool correction turns', async () => {
  const { hasDuplicateContinueNoProgress } = await loadWaveModule();
  const uniqueKey = 'database_lab:design_docs:database-lab/design/benchmark-plan.md';
  const instruction = {
    message: 'write benchmark-plan.md',
    metadata: { uniqueKey },
  };
  const scenarioState = {
    summary: {
  ...
  const firstAttempt = {
    issuedAt: 1000,
    lifecycleStatus: 'RUNNING',
    workspaceFingerprint: 'database-lab/design/README.md|database-lab/design/architecture.md|database-lab/design/storage-engine.md|database-lab/design/sql-compatibility.md',
    metadata: { uniqueKey },
  };

  assert.equal(hasDuplicateContinueNoProgress(instruction, scenarioState, [firstAttempt]), false);
  assert.equal(
...
test('database continue stale no-progress guard stops after provider turn ends without tools', async () => {
  ...
  assert.equal(hasStaleContinueNoProgress({ issuedAt }, scenarioState, issuedAt + 89_000), false);
  assert.equal(hasStaleContinueNoProgress({ issuedAt }, scenarioState, issuedAt + 91_000), true);
});

test('database continue stale no-progress guard ignores stale safe point but waits while provider turn is active', async () => {
  const { hasStaleContinueNoProgress } = await loadWaveModule();
  const issuedAt = 1000;
  ...
  assert.equal(hasStaleContinueNoProgress({ issuedAt }, staleSafePointState, issuedAt + 120_000), true);
  assert.equal(hasStaleContinueNoProgress({ issuedAt }, leasedState, issuedAt + 120_000), false);
});
...
test('database continue drift detection also observes visible tool activities from CLI projections', async () => {
  const { detectContinueInstructionDrift } = await loadWaveModule();
  const drift = detectContinueInstructionDrift(
    {
      summary: {
        visibleToolActivities: [
          {
            activityId: 'tool_visible_1',
            toolId: 'run_command',
            status: 'SUCCEEDED',
            startedAt: 250,
            argumentsSummary: 'node scripts/bench.js',
          },
        ],
      },
      task: {
  ...
  assert.match(drift, /forbidden tool/i);
  assert.match(drift, /run_command/i);
});
...
test('database continue drift detection accepts benchmark API companion module writes', async () => {
  const { detectContinueInstructionDrift } = await loadWaveModule();
  const drift = detectContinueInstructionDrift(
    {
      task: {
        toolInvocations: [
          ...
          },
          {
            ...
            status: 'SUCCEEDED',
            startedAt: 240,
            arguments: { path: 'database-lab/prototype/src/b-plus-tree-index.js' },
          },
          {
            invocationId: 'tool_4',
            toolId: 'write_file',
            status: 'SUCCEEDED',
            startedAt: 260,
            arguments: { path: 'database-lab/prototype/package.json' },
          },
        ],
      },
    },
...
test('database continue prompt advances to prototype top-level after design docs are complete', async () => {
  ...
  assert.match(instruction.message, /Leave the remaining files for the next repair pass: database-lab\/prototype\/README\.md, database-lab\/prototype\/scripts\/bench\.js/i);
  assert.match(instruction.message, /prefer write_file\.arguments\.content_json/i);
});

test('database continue prompt writes one prototype module per design scaffold turn', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const scenarioState = {
    summary: {
      lifecycleStatus: 'RUNNING',
...
test('database continue prompt repairs declared missing package entry before src module work', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-design-'));
  try {
    ...
    assert.match(instruction.message, /declared package entries point only at real files or are removed/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});

test('database continue prompt returns to prototype modules when benchmark ran before src modules exist', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-module-after-bench-'));
  try {
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'prototype'), { recursive: true });
    fs.writeFileSync(
      path.join(workspaceDir, 'database-lab', 'prototype', 'package.json'),
    ...
    assert.match(instruction.message, /Write the missing benchmark-critical implementation modules now/i);
    assert.match(instruction.message, /do not point it at an unwritten src\/index\.js/i);
    assert.match(instruction.message, /Do not emit .*run_command/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});

test('database continue prompt starts design phase with one design doc per turn', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const scenarioState = {
    summary: {
      lifecycleStatus: 'RUNNING',
  ...
  assert.match(instruction.message, /Do not try to finish all remaining design docs in this turn/i);
});
...
test('database verify repairs broken package entry refs before benchmark repair', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-verify-package-'));
  try {
    for (const relativePath of [
      'database-lab/design/README.md',
      'database-lab/design/architecture.md',
      ...
      const fullPath = path.join(workspaceDir, ...relativePath.split('/'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: 'db-prototype',
        main: 'src/index.js',
        scripts: { bench: 'node scripts/bench.js' },
      }, null, 2),
      'utf8',
    );
        ...
        executionSummary: {
          acceptance: {
            quality: {
              profileId: 'database_near_mysql_verify',
              verdict: 'failed',
              failedChecks: ['benchmark_scaffold_missing_required_metric_keys'],
            },
          },
        },
      },
    ...
    const instruction = deriveContinueMessage({ id: 'database-near-mysql-verify' }, scenarioState);
    assert.equal(instruction.metadata.phase, 'verify_package_entry_repair');
    assert.deepEqual(instruction.metadata.allowedTools, ['write_file']);
    assert.deepEqual(instruction.metadata.targetPaths, ['database-lab/prototype/package.json']);
    assert.match(instruction.message, /main:src\/index\.js/i);
    assert.match(instruction.message, /Remove optional main\/start entries/i);
    assert.match(instruction.message, /Do not emit .*run_command/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
...
test('database verify prompt repairs a failed benchmark before rerunning it', async () => {
  ...
  try {
    ...
    const instruction = deriveContinueMessage({ id: 'database-near-mysql-verify' }, scenarioState);
    assert.ok(['verify_bench_api_repair', 'verify_bench_failure_repair'].includes(instruction.metadata.phase));
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/scripts/bench.js'));
...
test('database verify prompt targets storage and index files when benchmark fails on an internal index API mismatch', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-verify-index-'));
  try {
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'prototype', 'scripts'), { recursive: true });
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'prototype', 'src'), { recursive: true });
    fs.writeFileSync(
    ...
    writeDatabaseCorePrototypeModules(workspaceDir, {
      'database-lab/prototype/src/storage-engine.js': [
        "const { BPlusTreeIndex } = require('./b-plus-tree-index.js');",
        'class StorageEngine {',
        '  constructor() {',
        '    this.indexes = new Map();',
        '    const primaryIndex = new BPlusTreeIndex();',
        "    this.indexes.set('users', primaryIndex);",
        '  }',
        '  select(id) {',
        "    const pkIndex = this.indexes.get('users');",
        '    return pkIndex.search(id);',
        '  }',
        '}',
        'module.exports = { StorageEngine };',
      ].join('\n'),
      'database-lab/prototype/src/b-plus-tree-index.js': [
        ...
        '  constructor() { this.entries = new Map(); }',
        '  insert(key, value) { this.entries.set(key, value); return value; }',
        '  lookup(key) { return this.entries.get(key) ?? null; }',
        '}',
        'module.exports = { BPlusTreeIndex };',
      ].join('\n'),
    });
        ...
        visibleToolActivities: [
          {
            ...
            activityId: 'tool_bench_fail',
            argumentsSummary: 'npm run bench -- --dry-run',
            resultSummary: 'TypeError: pkIndex.search is not a function',
          },
        ],
      },
      task: {
        toolInvocations: [
          {
            ...
            result: {
              exitCode: 1,
              stdout: '',
              stderr: 'TypeError: pkIndex.search is not a function\n    at StorageEngine.select (database-lab/prototype/src/storage-engine.js:9:20)',
            },
          },
        ],
      },
    ...
    assert.ok(['verify_bench_api_repair', 'verify_bench_failure_repair'].includes(instruction.metadata.phase));
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/src/storage-engine.js'));
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/src/b-plus-tree-index.js'));
    assert.match(instruction.message, /StorageEngine\/BPlusTreeIndex API mismatch/i);
    assert.match(instruction.message, /pkIndex\.search/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});
...
test('database verify prompt reruns benchmark when prior benchmark evidence is stale after bench changes', async () => {
  ...
  const instruction = deriveContinueMessage({ id: 'database-near-mysql-verify' }, scenarioState);
  const instructionText = typeof instruction === 'string' ? instruction : instruction.message;
  assert.equal(typeof instructionText, 'string');
  assert.match(instructionText, /evidence is now stale/i);
  assert.match(instructionText, /Do not reuse the old invocation ids yet/i);
  assert.match(instructionText, /rerun one real dry-run benchmark command/i);
  assert.match(instructionText, /only valid tool call is one run_command/i);
  assert.doesNotMatch(instructionText, /Do not rerun the benchmark first/i);
  assert.deepEqual(instruction.metadata.allowedTools, ['run_command']);
});

test('database verify prioritizes stale benchmark rerun over static API repair', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-verify-stale-'));
  try {
    for (const relativePath of [
      'database-lab/design/README.md',
      'database-lab/design/architecture.md',
    ...
    fs.writeFileSync(
      path.join(workspaceDir, 'database-lab', 'prototype', 'package.json'),
      JSON.stringify({ scripts: { bench: 'node scripts/bench.js' } }),
      'utf8',
    );
    fs.writeFileSync(path.join(workspaceDir, 'database-lab', 'prototype', 'README.md'), '# Prototype\n', 'utf8');
    fs.writeFileSync(
      path.join(workspaceDir, 'database-lab', 'prototype', 'scripts', 'bench.js'),
      [
        "const { StorageEngine } = require('../src/storage-engine');",
        'const storage = new StorageEngine();',
        'console.log(JSON.stringify({ status: "ok", metrics: storage.stats() }));',
      ].join('\n'),
      'utf8',
    );
    fs.writeFileSync(
      path.join(workspaceDir, 'database-lab', 'prototype', 'src', 'storage-engine.js'),
      'class StorageEngine { getStats() { return {}; } }\nmodule.exports = { StorageEngine };\n',
      'utf8',
    );
    for (const moduleName of ['buffer-pool', 'b-plus-tree-index', 'wal-manager', 'transaction-manager']) {
      fs.writeFileSync(
        path.join(workspaceDir, 'database-lab', 'prototype', 'src', `${moduleName}.js`),
        'module.exports = {};\n',
        'utf8',
      );
    }
    fs.writeFileSync(
      path.join(workspaceDir, 'quality', 'database-design.json'),
      JSON.stringify({ implementedModules: ['database-lab/prototype/src/storage-engine.js'] }),
      'utf8',
    );
    ...
    const scenarioState = {
      ...
      task: {
        toolInvocations: [
          {
            invocationId: 'tool_old_success',
            toolId: 'run_command',
            status: 'SUCCEEDED',
            arguments: { command: 'npm run bench -- --dry-run' },
            result: {
              exitCode: 0,
              stdout: JSON.stringify({
                status: 'ok',
    ...
    assert.match(instructionText, /rerun one real dry-run benchmark command/i);
    assert.match(instructionText, /only valid tool call is one run_command/i);
    assert.doesNotMatch(instructionText, /Static prototype inspection found/i);
    assert.doesNotMatch(instructionText, /Repair the cited prototype files/i);
    assert.deepEqual(instruction.metadata.allowedTools, ['run_command']);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
...
test('database verify prioritizes benchmark output repair after successful command evidence', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-verify-output-'));
  try {
    for (const relativePath of [
      'database-lab/design/README.md',
      'database-lab/design/architecture.md',
    ...
    fs.writeFileSync(
      ...
      [
        "const { StorageEngine } = require('../src/storage-engine');",
        'const storage = new StorageEngine();',
        'console.log(JSON.stringify({ status: "completed", summary: { ops: 1 } }));',
        'storage.stats();',
      ].join('\n'),
      'utf8',
    );
    ...
    const scenarioState = {
      ...
      task: {
        toolInvocations: [
          {
            invocationId: 'tool_success_bad_shape',
            toolId: 'run_command',
            status: 'SUCCEEDED',
            arguments: { command: 'npm run bench -- --dry-run' },
            result: {
              exitCode: 0,
              stdout: JSON.stringify({ status: 'completed', summary: { ops: 1 } }),
              stderr: '',
            },
          },
        ],
        ...
        task: {
          runtime: {
            schedulerUnits: {
              'AGENT-001': {
                invalidOutputErrors: [
                  'quality_gate_failed:benchmark_scaffold_missing_required_metric_keys',
                  'quality_gate_failed:benchmark_self_check_stale',
                ],
              },
            },
          },
        ...
        executionSummary: {
          acceptance: {
            quality: {
              profileId: 'database_near_mysql_verify',
              verdict: 'failed',
              failedChecks: ['benchmark_scaffold_missing_required_metric_keys', 'benchmark_self_check_stale'],
              requiredNextEvidence: ['repair benchmark output metrics before writing verification evidence'],
            },
          },
        },
      },
    ...
    const instruction = deriveContinueMessage({ id: 'database-near-mysql-verify' }, scenarioState);
    assert.equal(instruction.metadata.phase, 'verify_bench_scaffold_repair');
    assert.deepEqual(instruction.metadata.allowedTools, ['write_file']);
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/scripts/bench.js'));
    assert.match(instruction.message, /top-level status, summary, and metrics keys/i);
    assert.doesNotMatch(instruction.message, /benchmark scaffold and the current prototype APIs are not aligned/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});

test('database verify writes benchmark evidence after successful metrics before static API cleanup', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-verify-evidence-'));
  try {
    for (const relativePath of [
      'database-lab/design/README.md',
      'database-lab/design/architecture.md',
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'const storage = new StorageEngine();',
        'storage.stats();',
        'console.log(JSON.stringify({ status: "completed", summary: {}, metrics: { pagesWritten: 1, pagesRead: 1, writeDurationMs: 0, readDurationMs: 0, totalDurationMs: 0 } }));',
      ].join('\n'),
      'utf8',
    );
    fs.writeFileSync(
    ...
    const scenarioState = {
      ...
      task: {
        toolInvocations: [
          {
            invocationId: 'tool_success_metrics',
            toolId: 'run_command',
            status: 'SUCCEEDED',
            arguments: { command: 'npm run bench -- --dry-run' },
            result: {
      ...
      debug: {
        task: {
          runtime: {
            schedulerUnits: {
              'AGENT-001': {
                invalidOutputErrors: ['quality_gate_failed:missing_database_benchmark_result'],
              },
            },
          },
        },
        executionSummary: {
          acceptance: {
            quality: {
              profileId: 'database_near_mysql_verify',
              verdict: 'failed',
              failedChecks: ['missing_database_benchmark_result'],
              requiredNextEvidence: ['write quality/database-benchmark-result.json with resultFile and sourceInvocationId'],
            },
          },
        },
      },
    ...
    const instruction = deriveContinueMessage({ id: 'database-near-mysql-verify' }, scenarioState);
    const instructionText = typeof instruction === 'string' ? instruction : instruction.message;
    assert.notEqual(typeof instruction, 'string');
    assert.equal(instruction.metadata?.phase, 'verify_benchmark_evidence_write');
    assert.deepEqual(instruction.metadata?.allowedTools, ['write_file']);
    assert.deepEqual(instruction.metadata?.targetPaths, [
      'database-lab/prototype/results/bench-dry-run.json',
      'quality/database-benchmark-result.json',
    ]);
    assert.match(instructionText, /exactly two write_file calls/i);
    assert.match(instructionText, /quality\/database-benchmark-result\.json/i);
    assert.match(instructionText, /database-lab\/prototype\/results\/bench-dry-run\.json/i);
    assert.match(instructionText, /tool_success_metrics/i);
    assert.match(instructionText, /Do not emit read_file, list_files, search_files, create_folder, run_command, or delegate_subtask/i);
    assert.doesNotMatch(instructionText, /benchmark scaffold and the current prototype APIs are not aligned/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});

test('database verify repairs benchmark module wiring before writing evidence', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-verify-wiring-'));
  try {
    for (const relativePath of [
      'database-lab/design/README.md',
      'database-lab/design/architecture.md',
    ...
    fs.writeFileSync(
      ...
      [
        'const result = {',
        '  status: "success",',
        '  summary: { workload: "simulated" },',
        '  metrics: { pagesWritten: 1, pagesRead: 1, writeDurationMs: 0, readDurationMs: 0, totalDurationMs: 0 },',
        '};',
    ...
    const scenarioState = {
      ...
      task: {
        toolInvocations: [
          {
            invocationId: 'tool_simulated_success',
            toolId: 'run_command',
            status: 'SUCCEEDED',
            arguments: { command: 'npm run bench -- --dry-run', workingDirectory: 'database-lab/prototype' },
            result: {
              exitCode: 0,
              stdout: JSON.stringify({
                status: 'success',
        ...
        executionSummary: {
          acceptance: {
            quality: {
              ...
              failedChecks: [
                'missing_database_benchmark_result',
                'benchmark_not_wired_to_prototype_modules',
              ],
              requiredNextEvidence: [
                'database-lab/prototype/scripts/bench.js must import real modules under database-lab/prototype/src instead of placeholder-only logic',
              ],
            },
          },
        },
    ...
    assert.notEqual(typeof instruction, 'string');
    assert.equal(instruction.metadata?.phase, 'verify_bench_api_repair');
    assert.match(instruction.message, /not wired to prototype modules/i);
    ...
    assert.match(instruction.message, /Do not write database-lab\/prototype\/results\/bench-dry-run\.json or quality\/database-benchmark-result\.json/i);
    assert.doesNotMatch(instruction.message, /exactly two write_file calls/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});
...
test('database prototype diagnostics detect storage/index internal API drift before benchmark self-check', async () => {
  const { getDatabaseLabPrototypeCodeDiagnostics } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-index-drift-'));
  try {
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'prototype', 'scripts'), { recursive: true });
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'prototype', 'src'), { recursive: true });
    fs.writeFileSync(
    ...
    assert.ok(diagnostics.failedChecks.includes('storage_engine_index_contract_mismatch'));
    assert.ok(diagnostics.failedChecks.includes('storage_engine_index_missing_method:search'));
    assert.match(
      diagnostics.requiredNextEvidence.join('\n'),
      /storage-engine\.js and\/or database-lab\/prototype\/src\/b-plus-tree-index\.js/i,
    );
    assert.match(diagnostics.requiredNextEvidence.join('\n'), /search versus lookup/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});
...
test('database continue prompt expands typeerror benchmark repair to storage-engine when bench calls a non-default storageEngine variable', async () => {
  ...
  try {
    ...
    const allowedRepairPaths = new Set([
      ...instruction.metadata.targetPaths,
      ...instruction.metadata.allowedOptionalPaths,
    ]);
    assert.ok(allowedRepairPaths.has('database-lab/prototype/scripts/bench.js'));
    assert.ok(allowedRepairPaths.has('database-lab/prototype/src/buffer-pool.js'));
    assert.ok(allowedRepairPaths.has('database-lab/prototype/package.json'));
...
test('database continue prompt routes buffer-pool capacity benchmark failures into lifecycle repair', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-design-'));
  try {
    fs.mkdirSync(path.join(workspaceDir, 'brief'), { recursive: true });
    ...
    fs.writeFileSync(
      path.join(workspaceDir, 'database-lab', 'prototype', 'package.json'),
      JSON.stringify({ name: 'db-prototype', private: true, scripts: { bench: 'node scripts/bench.js' } }, null, 2),
      'utf8',
    );
    fs.writeFileSync(
      path.join(workspaceDir, 'database-lab', 'prototype', 'scripts', 'bench.js'),
      [
        ...
        "const { BufferPool } = require('../src/buffer-pool.js');",
        'async function dryRun() {',
        '  const bufferPool = new BufferPool({ capacity: 2 });',
        '  bufferPool.writePage(1, Buffer.alloc(8));',
        '  bufferPool.readPage(1);',
        '  return { status: "ok", summary: { writeCount: 1, readCount: 1 }, metrics: { pagesWritten: 1, pagesRead: 1, writeDurationMs: 1, readDurationMs: 1, totalDurationMs: 2 } };',
        '}',
        'dryRun().then((result) => process.stdout.write(JSON.stringify(result)));',
      ].join('\n'),
      'utf8',
    );
    const coreModuleFiles = writeDatabaseCorePrototypeModules(workspaceDir);
        ...
        toolInvocations: [
          {
            ...
            result: {
              stdout: '',
              stderr: '[bench] failed: Buffer pool is full and no evictable page found',
              exitCode: 1,
            },
          },
        ],
    ...
    const instruction = deriveContinueMessage({ id: 'database-near-mysql-design' }, scenarioState);
    assert.equal(instruction.metadata.phase, 'bench_buffer_pool_capacity_repair');
    assert.deepEqual(instruction.metadata.allowedTools, ['write_file', 'read_file']);
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/scripts/bench.js'));
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/src/buffer-pool.js'));
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/src/storage-engine.js'));
    assert.match(instruction.message, /Buffer pool is full and no evictable page found/i);
    assert.match(instruction.message, /release every page fetched during read and scan phases/i);
    assert.match(instruction.message, /Do not emit run_command in this repair turn/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
...
test('database design captures benchmark output before speculative peer module repair', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-design-'));
  try {
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'design'), { recursive: true });
    ...
    assert.equal(instruction.metadata.phase, 'benchmark_self_check');
    assert.ok(instruction.metadata.targetPaths.includes('database-lab/prototype/scripts/bench.js'));
    assert.match(instruction.message, /Run one real benchmark self-check now/i);
    assert.match(instruction.message, /Static inspection already sees likely prototype issues/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});
...
test('database prototype repair target prioritization keeps storage/index contract files together', async () => {
  const { getPrioritizedDatabasePrototypeRepairTargets } = await loadWaveModule();

  const targets = getPrioritizedDatabasePrototypeRepairTargets(
    {
      failedChecks: [
        'storage_engine_index_contract_mismatch',
        'storage_engine_index_missing_method:search',
      ],
    },
    [
      'database-lab/prototype/scripts/bench.js',
...
test('database design reserves one extra finalize continue after the base scenario budget is exhausted', async () => {
  ...
  const exhaustedAttempts = Array.from({ length: 18 }, (_, index) => ({
    metadata: {
      phase: index === 17 ? 'benchmark_self_check' : 'prototype_modules',
      allowedTools: ['write_file'],
    },
  }));
...
test('database design forces a real benchmark self-check when duplicate static repair would otherwise stall', async () => {
  const {
  ...
  assert.equal(shouldForceScenarioPackBenchmarkSelfCheck({ id: 'database-near-mysql-design' }, scenarioState, []), true);
  assert.equal(shouldForceScenarioPackBenchmarkSelfCheck({ id: 'database-near-mysql-verify' }, scenarioState, []), false);
  assert.equal(
    shouldForceScenarioPackBenchmarkSelfCheck(
      { id: 'database-near-mysql-design' },
      scenarioState,
      [{ metadata: { phase: 'benchmark_self_check' } }],
    ),
    false,
  );

  const instruction = buildScenarioPackBenchmarkSelfCheckInstruction('force benchmark now');
  assert.equal(instruction.metadata.phase, 'benchmark_self_check');
  assert.deepEqual(instruction.metadata.allowedTools, ['run_command']);
  assert.match(instruction.message, /npm run bench -- --dry-run/);

  const staleScenarioState = {
    ...scenarioState,
    debug: {
      executionSummary: {
        acceptance: {
          quality: {
            ...
            failedChecks: ['benchmark_self_check_stale'],
            requiredNextEvidence: [
              'rerun a successful dry-run benchmark command from database-lab/prototype after the latest bench.js or prototype/src changes',
            ],
          },
        },
      },
    ...
    task: {
      toolInvocations: [{
        ...
        result: {
          stdout: '{"status":"ok","summary":{"writeCount":1,"readCount":1},"metrics":{"pagesWritten":1,"pagesRead":1,"writeDurationMs":1,"readDurationMs":1,"totalDurationMs":2}}',
          stderr: '',
          exitCode: 0,
        },
      }],
  ...
  assert.equal(
    canIssueContinue(
      { id: 'database-near-mysql-design' },
      Array.from({ length: 18 }, (_, index) => ({ metadata: { phase: index === 3 ? 'benchmark_self_check' : 'prototype_contract_repair' } })),
      instruction,
    ),
    true,
  );
...
test('database prototype diagnostics flag WAL Date.now timestamp written through UInt32', async () => {
  const { getDatabaseLabPrototypeCodeDiagnostics } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-wave-wal-u32-'));
  try {
    const coreModuleFiles = writeDatabaseCorePrototypeModules(workspaceDir, {
      'database-lab/prototype/src/wal-manager.js': [
        'class WALManager {',
        '  constructor() { this.records = []; }',
        '  append(type, txnId, data = null) { const record = { type, txnId, data, timestamp: Date.now() }; this.records.push(record); return record; }',
        '  serialize(record) {',
        '    const buf = Buffer.alloc(20);',
        '    buf.writeUInt32LE(record.timestamp, 12);',
        '    return buf;',
        '  }',
        '  close() { return true; }',
        '}',
    ...
    fs.writeFileSync(
      ...
      [
        "const { WALManager } = require('../src/wal-manager.js');",
        'function dryRun() { const wal = new WALManager(); wal.append(1, 1, {}); return { status: "ok", summary: {}, metrics: { pagesWritten: 1, pagesRead: 1, writeDurationMs: 1, readDurationMs: 1, totalDurationMs: 2 } }; }',
        'module.exports = { dryRun };',
      ].join('\n'),
      'utf8',
    );
    ...
    assert.ok(diagnostics.failedChecks.includes('wal_timestamp_uint32_overflow_risk'));
    assert.match(diagnostics.requiredNextEvidence.join('\n'), /Date\.now\(\) exceeds uint32 range/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});
...
test('database continue prompt writes design manifest after artifact benchmark self-check passes', async () => {
  const { deriveContinueMessage } = await loadWaveModule();
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scc-db-design-manifest-'));
  try {
    fs.mkdirSync(path.join(workspaceDir, 'brief'), { recursive: true });
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'design'), { recursive: true });
    fs.mkdirSync(path.join(workspaceDir, 'database-lab', 'prototype', 'scripts'), { recursive: true });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "const { TransactionManager } = require('../src/transaction-manager.js');",
        'const engine = new StorageEngine(".tmp");',
        'const pool = new BufferPool({ storageEngine: engine });',
        'const index = new BPlusTreeIndex();',
        'const wal = new WALManager(".tmp");',
        'const tx = new TransactionManager({ storageEngine: engine, bufferPool: pool, walManager: wal, indexManager: index });',
        'void tx;',
        'engine.writePage(1, Buffer.alloc(8));',
        'pool.writePage(1, Buffer.alloc(8));',
        'pool.readPage(1);',
        'index.insert(1, { pageId: 1 });',
        'wal.appendEntry({ type: "insert" });',
        'process.stdout.write(JSON.stringify({ status: "ok", summary: { writeCount: 1, readCount: 1 }, metrics: { pagesWritten: 1, pagesRead: 1, writeDurationMs: 1, readDurationMs: 1, totalDurationMs: 2 } }));',
      ].join('\n'),
      'utf8',
    );
    const coreModuleFiles = writeDatabaseCorePrototypeModules(workspaceDir);
    ...
    assert.match(instruction.message, /The dry-run benchmark already passed/i);
    assert.match(instruction.message, /Write only quality\/database-design\.json/i);
  } finally {
    fs.rmSync(workspaceDir, { recursive: true, force: true });
  }
});
