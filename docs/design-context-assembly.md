...
```
┌─────────────────────────────────────────────────────────────┐
│  Layer 1: System Instructions (固定，~200 tokens)            │
│  - Agent 身份与行为约束                                       │
├─────────────────────────────────────────────────────────────┤
│  Layer 2: Skill MetaData (轻量，~300 tokens)                 │
│  - 可用 Skill 列表（仅标题+描述+成功率）                      │
│  - Agent 可调用 use_skill 工具加载完整内容                     │
├─────────────────────────────────────────────────────────────┤
│  Layer 3: File State Table (~1000 tokens，动态)              │
│  - 当前已知文件状态（路径+关键内容摘要）                       │
│  - 解决"猜代码"问题                                          │
├─────────────────────────────────────────────────────────────┤
│  Layer 4: Conversation History (剩余全部 tokens)             │
│  - 结构化对话历史                                            │
│  - 智能截断，优先保留最近事件                                 │
├─────────────────────────────────────────────────────────────┤
│  Layer 5: Current Input (用户当前输入)                        │
│  - 当前用户消息或工具结果                                     │
└─────────────────────────────────────────────────────────────┘
```
